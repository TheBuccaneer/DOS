...
# Thin environment wrapper around run_chunk_budget_screen.py. The Python runner
...
PYTHON_RUNNER="${SCRIPT_DIR}/run_chunk_budget_screen.py"
...
if [[ ! -f "$VENV_ACTIVATE" || ! -r "$VENV_ACTIVATE" ]]; then
  ...
  echo "  VENV_PATH=/pfad/zu/.venv bash run_chunk_budget_screen.sh ..." >&2
  exit 1
fi
...
LOGDIR="${PROJECT_ROOT}/new/logs/chunk_budget_screen"
...
LOGFILE="${LOGDIR}/run_chunk_budget_screen_${TIMESTAMP}_pid${BASHPID}.log"
if ! : > "$LOGFILE"; then
