...
# enables logging, and forwards arguments unchanged. Mirrors the audited
# new/scripts/prefill_confirmation/run_prefill_confirmation.sh contract,
# adapted for this campaign's own filenames/log directory.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
PYTHON_RUNNER="${SCRIPT_DIR}/run_server_waiting_confirmation.py"
SERVER_SCRIPT="${SCRIPT_DIR}/run_server_waiting_server.sh"
...
for arg in "$@"; do
  case "$arg" in
    --official-run|--smoke-test|--diagnostic-pair-only) REAL_RUN=1 ;;
  esac
done
...
if (( REAL_RUN )); then
  if [[ -z "$VLLM_PATH" ]]; then
    echo "Fehler: vllm ist für Smoke-/Diagnostic-Pair-/Official-Runs im aktivierten Venv erforderlich." >&2
    exit 1
  fi
  ...
  if [[ -z "${VLLM_API_KEY:-}" ]]; then
    echo "Fehler: VLLM_API_KEY muss für Smoke-/Diagnostic-Pair-/Official-Runs explizit gesetzt sein." >&2
    exit 1
  fi
  export VLLM_API_KEY
fi
...
LOGDIR="${PROJECT_ROOT}/new/logs/server_waiting_confirmation"
mkdir -p "$LOGDIR"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOGFILE="${LOGDIR}/run_server_waiting_confirmation_${TIMESTAMP}_pid${BASHPID}.log"
: > "$LOGFILE"
