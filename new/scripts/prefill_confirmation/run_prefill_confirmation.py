#!/usr/bin/env python3
"""
run_prefill_confirmation.py -- frozen confirmatory Prefill-Confirmation runner.

Executes one of two independent 96-episode model campaigns (Llama or Qwen)
from the audited prefill_confirmation_schedule.* bundle. The scientific
request, trigger, timing-instrumentation, server lifecycle, stabilization,
resume-depth, environment-fingerprint, and integrity machinery is inherited
unchanged from the audited Schema-5 Prefill-Trigger-Sweep timing runner.

Frozen grid per model: offload {0,8,12} x concurrency {4,8} x repeats {1..8},
paired no_burst/prefill_burst conditions, trigger_after_decode_tokens=16.
...
# ============================================================================
# Frozen official Prefill-Confirmation contract
#
# This is a fully separate, confirmatory paired design -- derived
...
# Model registry: the only model-dependent facts. Each invocation loads
# exactly one independently fingerprinted model bundle.
...
OFFICIAL_FINGERPRINTS: dict[str, str] = {
    "llama": "sha256:c0712c24bfc13e15721771f747679ee17b94737c71dcb8ead3f4f9b3d42b1b9d",
    "qwen": "sha256:f272736db61d70091fa7767a360e8759340a719227bf656dd667a64e0a836bfb",
}
OFFICIAL_SEED_BY_MODEL: dict[str, int] = {"llama": 20260718, "qwen": 20260718}
OFFICIAL_EPISODE_COUNT_PER_MODEL = 96
OFFICIAL_BLOCK_COUNT_PER_MODEL = 48
OFFICIAL_TRIGGER_POSITIONS = [16]
OFFICIAL_CONCURRENCY_VALUES = [4, 8]
OFFICIAL_OFFLOAD_VALUES = [0, 8, 12]
OFFICIAL_MAX_NUM_BATCHED_TOKENS = 2048
OFFICIAL_REPEATS = 8
OFFICIAL_INCLUDED_REPEATS = list(range(1, OFFICIAL_REPEATS + 1))
OFFICIAL_PLANNED_REPEAT_CHECKPOINTS = [8, 12, 16]

BURST_CONDITION = "prefill_burst"
OFFICIAL_CONDITIONS = ["no_burst", BURST_CONDITION]
OFFICIAL_STATES = [
    {"offload_gb": 0, "state_label": "low"},
    {"offload_gb": 8, "state_label": "intermediate"},
    {"offload_gb": 12, "state_label": "high"},
]
STATE_LABEL_BY_OFFLOAD: dict[int, str] = {
    0: "low", 8: "intermediate", 12: "high",
}
...
BLOCK_SIZE = len(OFFICIAL_CONDITIONS)
BLOCKS_PER_MODEL = OFFICIAL_BLOCK_COUNT_PER_MODEL
EPISODES_PER_MODEL = OFFICIAL_EPISODE_COUNT_PER_MODEL
TOTAL_RESTART_MARKERS_PER_MODEL = BLOCKS_PER_MODEL
...
RUN_MODE_MARKER_FILENAME = ".prefill_confirmation_run_mode"

# ============================================================================
# Portable paths (derived from this file's own location, not hardcoded)
# Expected location: <PROJECT_ROOT>/new/scripts/prefill_confirmation/run_prefill_confirmation.py
# ============================================================================

SCRIPT_PATH = Path(__file__).resolve()
PROJECT_ROOT = SCRIPT_PATH.parents[3]
DEFAULT_SCHEDULE_DIR_ROOT = PROJECT_ROOT / "new" / "runs" / "prefill_confirmation"
...
def check_audit(audit_text: str, expected_fingerprint: object, model_key: str) -> list[str]:
    errors: list[str] = []
    ...
    expected_campaign = MODEL_REGISTRY[model_key]["campaign_name"]
    required_patterns = (
        (r"^schema_version:\s*1\s*$", "schema_version: 1"),
        (r"^design_version:\s*prefill-confirmation-v1\s*$", "design_version"),
        (rf"^campaign_name:\s*{re.escape(expected_campaign)}\s*$", "campaign_name"),
        (rf"^model_key:\s*{re.escape(model_key)}\s*$", "model_key"),
        (r"^block count:\s*48\s*$", "block count: 48"),
        (r"^episode count:\s*96\s*$", "episode count: 96"),
...
def check_official_contract(json_obj: dict, model_key: str) -> list[str]:
    ...
    check_eq("offload_values", OFFICIAL_OFFLOAD_VALUES)
    check_eq("concurrency_values", OFFICIAL_CONCURRENCY_VALUES)
    check_eq("trigger_positions", OFFICIAL_TRIGGER_POSITIONS)
    check_eq("conditions", OFFICIAL_CONDITIONS)
    check_eq("initial_repeats", OFFICIAL_REPEATS)
    check_eq("included_repeats", OFFICIAL_INCLUDED_REPEATS)
    check_eq("planned_repeat_checkpoints", OFFICIAL_PLANNED_REPEAT_CHECKPOINTS)
    check_eq("block_count", OFFICIAL_BLOCK_COUNT_PER_MODEL)
    ...
    check_eq("extension_policy", OFFICIAL_EXTENSION_POLICY)
    ...
    if not is_valid_fingerprint_format(fp):
        ...
        expected_fp = OFFICIAL_FINGERPRINTS[model_key]
        if fp != expected_fp:
            errors.append(
                f"schedule_fingerprint {fp!r} does not match frozen Prefill-Confirmation "
                f"fingerprint for model_key={model_key!r} ({expected_fp!r})"
            )
    return errors
...
# schedule), each Prefill-Confirmation bundle belongs to exactly one
...
def check_structural_schedule(
    ...
    if len(episodes) != OFFICIAL_EPISODE_COUNT_PER_MODEL:
        errors.append(
            f"{ctx_model}: expected {OFFICIAL_EPISODE_COUNT_PER_MODEL} episodes, found {len(episodes)}"
        )

    ids = [ep.episode_id for ep in episodes]
    seeds = [ep.episode_seed for ep in episodes]
    if len(ids) != len(set(ids)):
        errors.append(f"{ctx_model}: duplicate episode_id(s)")
    if len(seeds) != len(set(seeds)):
        errors.append(f"{ctx_model}: duplicate episode_seed(s)")

    for ep in episodes:
        ctx = f"{ctx_model}, episode={ep.episode_id}"
        if ep.model_key != model_key:
            errors.append(f"{ctx}: wrong model_key")
        if ep.model_id != expected_model_id:
            errors.append(f"{ctx}: wrong model_id")
        expected_state = STATE_LABEL_BY_OFFLOAD.get(ep.offload_gb)
        if expected_state is None or ep.state_label != expected_state:
            errors.append(f"{ctx}: invalid offload/state pair")
        if ep.concurrency not in OFFICIAL_CONCURRENCY_VALUES:
            errors.append(f"{ctx}: invalid concurrency {ep.concurrency!r}")
        if ep.trigger_after_decode_tokens != 16:
            errors.append(f"{ctx}: trigger must be 16")
        if ep.condition not in OFFICIAL_CONDITIONS:
            errors.append(f"{ctx}: invalid condition")
        if type(ep.repeat) is not int or ep.repeat not in OFFICIAL_INCLUDED_REPEATS:
            errors.append(f"{ctx}: invalid repeat {ep.repeat!r}")
        if ep.random_seed != schedule_seed:
            errors.append(f"{ctx}: random_seed mismatch")
        if ep.max_num_batched_tokens != OFFICIAL_MAX_NUM_BATCHED_TOKENS:
            errors.append(f"{ctx}: max_num_batched_tokens mismatch")
        for field_name, expected in OFFICIAL_VICTIM_CONFIGURATION.items():
            if getattr(ep, field_name) != expected:
                errors.append(f"{ctx}: {field_name} mismatch")
        for field_name, expected in OFFICIAL_BURST_CONFIGURATION.items():
            if getattr(ep, field_name) != expected:
                errors.append(f"{ctx}: {field_name} mismatch")
        expected_id = (
            f"{model_key}_off{ep.offload_gb}_conc{ep.concurrency}_trigger16_"
            f"{ep.condition}_rep{ep.repeat:02d}"
        )
        if ep.episode_id != expected_id:
            errors.append(f"{ctx}: episode_id != {expected_id!r}")
        expected_block_id = f"{model_key}_off{ep.offload_gb}_conc{ep.concurrency}_rep{ep.repeat:02d}"
        if ep.block_id != expected_block_id:
            errors.append(f"{ctx}: block_id != {expected_block_id!r}")
        if ep.episode_seed != derive_seed(str(schedule_seed), ep.episode_id):
            errors.append(f"{ctx}: episode_seed derivation mismatch")
        if ep.victim_workload_seed != derive_seed(str(schedule_seed), model_key, "victim", str(ep.repeat)):
            errors.append(f"{ctx}: victim_workload_seed derivation mismatch")
        if ep.burst_workload_seed != derive_seed(str(schedule_seed), model_key, "burst", str(ep.repeat)):
            errors.append(f"{ctx}: burst_workload_seed derivation mismatch")
        if ep.victim_workload_seed == ep.burst_workload_seed:
            errors.append(f"{ctx}: victim and burst seeds coincide")

    # Contiguous paired blocks in exact deterministic repeat/cell order.
    blocks: list[list[Episode]] = []
    idx = 0
    seen_blocks: set[str] = set()
    while idx < len(episodes):
        bid = episodes[idx].block_id
        end = idx
        while end < len(episodes) and episodes[end].block_id == bid:
            end += 1
        run = episodes[idx:end]
        if bid in seen_blocks:
            errors.append(f"{ctx_model}: block {bid!r} reappears non-contiguously")
        seen_blocks.add(bid)
        blocks.append(run)
        idx = end

    if len(blocks) != OFFICIAL_BLOCK_COUNT_PER_MODEL:
        errors.append(f"{ctx_model}: expected 48 blocks, found {len(blocks)}")

    base_cells = [(o, c) for o in OFFICIAL_OFFLOAD_VALUES for c in OFFICIAL_CONCURRENCY_VALUES]
    expected_block_ids: list[str] = []
    expected_first_by_cell_repeat: dict[tuple[int, int, int], str] = {}
    for offload, conc in base_cells:
        rng = random.Random(derive_seed(str(schedule_seed), model_key, "condition-order", str(offload), str(conc)))
        firsts = ["no_burst"] * 4 + [BURST_CONDITION] * 4
        rng.shuffle(firsts)
        for repeat, first in enumerate(firsts, 1):
            expected_first_by_cell_repeat[(offload, conc, repeat)] = first
    for repeat in OFFICIAL_INCLUDED_REPEATS:
        cells = list(base_cells)
        random.Random(derive_seed(str(schedule_seed), model_key, "block-order", str(repeat))).shuffle(cells)
        expected_block_ids.extend(
            f"{model_key}_off{o}_conc{c}_rep{repeat:02d}" for o, c in cells
        )

    actual_block_ids = [run[0].block_id for run in blocks if run]
    if actual_block_ids != expected_block_ids:
        errors.append(f"{ctx_model}: block order does not match deterministic derivation")

    first_counts: dict[tuple[int, int], dict[str, int]] = {
        cell: {"no_burst": 0, BURST_CONDITION: 0} for cell in base_cells
    }
    for run in blocks:
        if len(run) != 2:
            errors.append(f"{ctx_model}: block has {len(run)} episodes, expected 2")
            continue
        first, second = run
        if [e.order_in_block for e in run] != [1, 2]:
            errors.append(f"{ctx_model}: block {first.block_id!r} order sequence invalid")
        if [e.restart_server_before_block for e in run] != [1, 0]:
            errors.append(f"{ctx_model}: block {first.block_id!r} restart sequence invalid")
        if {e.condition for e in run} != set(OFFICIAL_CONDITIONS):
            errors.append(f"{ctx_model}: block {first.block_id!r} condition pair invalid")
        same_fields = (
        ...
        for field_name in same_fields:
            if getattr(first, field_name) != getattr(second, field_name):
                errors.append(f"{ctx_model}: block {first.block_id!r} differs in {field_name}")
        expected_first = expected_first_by_cell_repeat.get((first.offload_gb, first.concurrency, first.repeat))
        if first.condition != expected_first:
            errors.append(f"{ctx_model}: block {first.block_id!r} first condition derivation mismatch")
        if any(e.condition_first_in_block != first.condition for e in run):
            errors.append(f"{ctx_model}: block {first.block_id!r} condition_first_in_block mismatch")
        cell_key = (first.offload_gb, first.concurrency)
        if cell_key in first_counts and first.condition in first_counts[cell_key]:
            first_counts[cell_key][first.condition] += 1
        else:
            errors.append(
                f"{ctx_model}: block {first.block_id!r} has an invalid cell/first-condition "
                f"combination {cell_key!r}/{first.condition!r}"
            )

    for cell, counts in sorted(first_counts.items()):
        if counts != {"no_burst": 4, BURST_CONDITION: 4}:
            errors.append(f"{ctx_model}: cell {cell} first-condition balance {counts} != 4/4")

    # Every repeat round has every cell exactly once and shared workload seeds.
    for repeat in OFFICIAL_INCLUDED_REPEATS:
        eps = [e for e in episodes if e.repeat == repeat]
        block_cells = {(e.offload_gb, e.concurrency) for e in eps if e.order_in_block == 1}
        if block_cells != set(base_cells) or len(eps) != 12:
            errors.append(f"{ctx_model}: repeat {repeat} does not contain all six paired cells")
        if len({e.victim_workload_seed for e in eps}) != 1:
            errors.append(f"{ctx_model}: repeat {repeat} victim seed not constant")
        if len({e.burst_workload_seed for e in eps}) != 1:
            errors.append(f"{ctx_model}: repeat {repeat} burst seed not constant")

    if sum(e.restart_server_before_block for e in episodes) != TOTAL_RESTART_MARKERS_PER_MODEL:
        errors.append(f"{ctx_model}: restart marker count mismatch")
    return errors
...
def load_and_validate_bundle(
    ...
    try:
        ...
        json_obj = load_json_bundle(paths["prefill_confirmation_schedule.json"])
        csv_fieldnames, csv_rows = load_csv_bundle(paths["prefill_confirmation_schedule.csv"])
        audit_text = load_audit_text(paths["prefill_confirmation_schedule_audit.txt"])
    ...
    errors.extend(check_audit(audit_text, fingerprint, model_key))
...
RUNNER_VERSION = "run_prefill_confirmation-v1-timing1"
...
class RealEnvironmentProbe:
    ...
    def gather(self, schedule_dir: Path) -> dict:
        ...
        file_hash_targets = {
            "run_prefill_confirmation.py": SCRIPT_PATH,
            "run_prefill_confirmation.sh": SCRIPT_PATH.parent / "run_prefill_confirmation.sh",
            "run_server.sh": SCRIPT_PATH.parent / "run_server.sh",
            "prefill_confirmation_schedule.json": schedule_dir / "prefill_confirmation_schedule.json",
            "prefill_confirmation_schedule.csv": schedule_dir / "prefill_confirmation_schedule.csv",
            "prefill_confirmation_schedule_audit.txt": schedule_dir / "prefill_confirmation_schedule_audit.txt",
        }
...
class FakeEnvironmentProbe:
    ...
    def __init__(self, env: dict | None = None) -> None:
        self.env: dict = env if env is not None else {
            ...
            "file_hashes": {
                "run_prefill_confirmation.py": "a" * 64,
                "run_prefill_confirmation.sh": "b" * 64,
                "run_server.sh": "c" * 64,
                "prefill_confirmation_schedule.json": "d" * 64,
                "prefill_confirmation_schedule.csv": "e" * 64,
                "prefill_confirmation_schedule_audit.txt": "0" * 64,
            },
        }
...
async def run_official_campaign(
    *,
    ...
    if resume:
        ...
        # integrity_manifest.json, .prefill_confirmation_run_mode) or unknown ones --
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="run_prefill_confirmation.py",
        description=(
            "Prefill-Confirmation runner (frozen confirmatory campaign: schedule-bundle validation, "
            "resume-depth checks, self-test, dry-run, real execution of a "
            "single selectable smoke-test block, and the complete "
            "official 48-block/96-episode confirmation campaign). --official-run is "
            "enabled."
        ),
    )
    ...
    mode_group.add_argument(
        ...
        help="Really run the complete frozen confirmation campaign (48 server blocks, "
        "96 regular episodes) in exact schedule order: strict environment "
    ...
    parser.add_argument(
        ...
        f"(default: new/runs/prefill_confirmation/<model-key>).",
    )
    parser.add_argument(
        "--output-dir", type=Path, default=None,
        help="Result output directory for --official-run/--smoke-test "
        "(default: new/runs/prefill_confirmation/<model-key>/results/<mode>). "
    ...
    parser.add_argument(
        ...
        "exact block_id to execute, e.g. 'llama_off12_conc8_rep01'.",
    )
    parser.add_argument(
...
def run_self_test() -> int:
    ...
    print("Prefill-Confirmation self-test (no GPU, no network)")
    print("=" * 60)

    # Trigger callback still uses actually received token counts.
    ev = asyncio.Event()
    crossing = _ActiveWaveCrossing(request_index=0)
    cb = make_threshold_callback(0, 16, crossing, ev)
    for cumulative in (1, 8, 15):
        cb(cumulative, cumulative * 100)
    check("trigger=16 does not fire before threshold", not ev.is_set())
    cb(18, 1800)
    check("trigger=16 fires on the batch crossing the threshold", ev.is_set() and crossing.received_token_count_at_crossing == 18)

    llama_payload = build_canonical_payload_for_selftest("llama")
    qwen_payload = build_canonical_payload_for_selftest("qwen")
    check("llama self-test fingerprint equals frozen bundle", llama_payload["schedule_fingerprint"] == OFFICIAL_FINGERPRINTS["llama"])
    check("qwen self-test fingerprint equals frozen bundle", qwen_payload["schedule_fingerprint"] == OFFICIAL_FINGERPRINTS["qwen"])
    check("model fingerprints differ", OFFICIAL_FINGERPRINTS["llama"] != OFFICIAL_FINGERPRINTS["qwen"])

    for model_key, payload in (("llama", llama_payload), ("qwen", qwen_payload)):
        same_errors = check_official_contract(payload, model_key)
        other = "qwen" if model_key == "llama" else "llama"
        cross_errors = check_official_contract(payload, other)
        eps = [parse_episode_from_json(e) for e in payload["episodes"]]
        check(f"{model_key}: official top-level contract passes", not same_errors)
        check(f"{model_key}: foreign model contract is rejected", bool(cross_errors))
        check(f"{model_key}: exactly 96 episodes", len(eps) == 96)
        check(f"{model_key}: exactly 48 blocks", len({e.block_id for e in eps}) == 48)
        check(f"{model_key}: structural schedule validation passes", not check_structural_schedule(eps, payload["seed"], model_key))
        check(f"{model_key}: only trigger 16", {e.trigger_after_decode_tokens for e in eps} == {16})
        check(f"{model_key}: offloads 0/8/12", {e.offload_gb for e in eps} == {0, 8, 12})
        check(f"{model_key}: concurrencies 4/8", {e.concurrency for e in eps} == {4, 8})
        check(f"{model_key}: repeats 1..8", {e.repeat for e in eps} == set(range(1, 9)))
    ...
    llama_workload_seeds = {e.victim_workload_seed for e in llama_eps} | {e.burst_workload_seed for e in llama_eps}
    qwen_workload_seeds = {e.victim_workload_seed for e in qwen_eps} | {e.burst_workload_seed for e in qwen_eps}
    check("Llama/Qwen workload seed streams are disjoint", llama_workload_seeds.isdisjoint(qwen_workload_seeds))
    check("unknown model key hard-fails", bool(check_official_contract(llama_payload, "unknown")))
    check("API key is absent from schedule payloads", "api_key" not in json.dumps(llama_payload).lower())
...
def build_canonical_payload_for_selftest(model_key: str) -> dict:
    """Rebuild the frozen confirmation payload independently, without
    importing the schedule generator or reading files."""
    if model_key not in MODEL_REGISTRY:
    ...
    for offload, conc in base_cells:
        rng = random.Random(derive_seed(str(seed), model_key, "condition-order", str(offload), str(conc)))
        values = ["no_burst"] * 4 + [BURST_CONDITION] * 4
        rng.shuffle(values)
        first_by_cell[(offload, conc)] = values

    episodes: list[dict] = []
    for repeat in OFFICIAL_INCLUDED_REPEATS:
        cells = list(base_cells)
        random.Random(derive_seed(str(seed), model_key, "block-order", str(repeat))).shuffle(cells)
        victim_seed = derive_seed(str(seed), model_key, "victim", str(repeat))
        burst_seed = derive_seed(str(seed), model_key, "burst", str(repeat))
        for offload, conc in cells:
            first = first_by_cell[(offload, conc)][repeat - 1]
            second = "no_burst" if first == BURST_CONDITION else BURST_CONDITION
            block_id = f"{model_key}_off{offload}_conc{conc}_rep{repeat:02d}"
            for pos, condition in enumerate((first, second), 1):
                episode_id = f"{model_key}_off{offload}_conc{conc}_trigger16_{condition}_rep{repeat:02d}"
                episodes.append({
                    "episode_id": episode_id,
                    "model_key": model_key,
                    "model_id": model_id,
                    "offload_gb": offload,
                    "state_label": STATE_LABEL_BY_OFFLOAD[offload],
                    "concurrency": conc,
                    ...
                    **OFFICIAL_BURST_CONFIGURATION,
                    ...
                    "condition_first_in_block": first,
    ...
    payload = {
        ...
        "campaign_name": MODEL_REGISTRY[model_key]["campaign_name"],
        "model_key": model_key,
        "model_id": model_id,
        "seed": seed,
        "offload_values": list(OFFICIAL_OFFLOAD_VALUES),
        "concurrency_values": list(OFFICIAL_CONCURRENCY_VALUES),
        "trigger_positions": list(OFFICIAL_TRIGGER_POSITIONS),
        "conditions": list(OFFICIAL_CONDITIONS),
        "initial_repeats": OFFICIAL_REPEATS,
        "included_repeats": list(OFFICIAL_INCLUDED_REPEATS),
        "planned_repeat_checkpoints": list(OFFICIAL_PLANNED_REPEAT_CHECKPOINTS),
        "block_count": OFFICIAL_BLOCK_COUNT_PER_MODEL,
        "episode_count": len(episodes),
        "victim_configuration": dict(OFFICIAL_VICTIM_CONFIGURATION),
        "burst_configuration": dict(OFFICIAL_BURST_CONFIGURATION),
        "stabilization_configuration": dict(OFFICIAL_STABILIZATION_CONFIGURATION),
        "extension_policy": dict(OFFICIAL_EXTENSION_POLICY),
        "episodes": episodes,
    }
    payload["schedule_fingerprint"] = recompute_fingerprint(payload)
    return payload
