...
[[ "$OFFLOAD_GB" =~ ^(0|12)$ ]] || {
  echo "Fehler: offload_gb muss im eingefrorenen Design 0 oder 12 sein." >&2; exit 1;
}
...
LOGDIR="${PROJECT_ROOT}/new/logs/prefill_trigger_sweep/server"
mkdir -p "$LOGDIR"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOGFILE="${LOGDIR}/server_${MODEL_KEY}_offload${OFFLOAD_GB}_${TIMESTAMP}.log"

{
  echo "Prefill-Trigger-Sweep vLLM server"
