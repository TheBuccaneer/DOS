#!/usr/bin/env python3
"""
run_prefill_screen.py -- frozen exploratory Prefill-Screen runner.

This runner executes a fully separate 24-episode screening campaign:
...
Each of the six state blocks follows the same hardened protocol used by
the frozen Phase-A infrastructure: server start, readiness validation,
one excluded stabilization run, health gate, fixed cooldown, four
regular episodes in schedule order, atomic outputs, and verified
process-group shutdown with port-release polling.
...
CLI modes:
  ...
  --smoke-test    Execute exactly one selected four-episode block.
  --official-run Execute all six blocks / 24 regular episodes.

Examples:
  python3 run_prefill_screen.py --self-test
  python3 run_prefill_screen.py --dry-run \
      --schedule-dir /path/to/new/runs/prefill_screen
  VLLM_API_KEY=... GPU_DEVICE=0 python3 run_prefill_screen.py --official-run \
      --schedule-dir /path/to/new/runs/prefill_screen \
      --output-dir /path/to/new/runs/prefill_screen/results/official
  VLLM_API_KEY=... GPU_DEVICE=0 python3 run_prefill_screen.py --smoke-test \
      --smoke-block llama_block01_low \
      --schedule-dir /path/to/new/runs/prefill_screen \
      --output-dir /path/to/new/runs/prefill_screen/results/smoke
...
# ============================================================================
# Frozen official Prefill-Screen contract
#
...
OFFICIAL_FINGERPRINT = (
    "sha256:a2f3fa1ddb56c1a8e60fe008603514c1098cfadbf843818fbae5325e9cf59232"
)
OFFICIAL_SEED = 20260716
OFFICIAL_EPISODE_COUNT = 24
OFFICIAL_MODELS = ["llama"]
OFFICIAL_REPEATS = 3
OFFICIAL_CONCURRENCIES = [4, 8]
# Prefill-heavy, bounded burst condition -- replaces Phase A's mixed
# Phase A's mixed 256/256 burst condition. Kept as a single named constant so
# every burst-shape/-count/-interval check below derives from one place.
...
# Prefill-heavy, bounded burst: large input, small output -- see module
# docstring for the scientific rationale (vs. Phase A's mixed 256/256).
...
# 6 blocks total: repeat 1 (low,high), repeat 2 (high,low), repeat 3 (low,high).
EXPECTED_STATE_SEQUENCE: list[str] = [
    "low", "high", "high", "low", "low", "high",
]
BLOCK_SIZE = len(OFFICIAL_CONCURRENCIES) * len(OFFICIAL_CONDITIONS)  # 4
BLOCKS_PER_MODEL = 2 * OFFICIAL_REPEATS  # 6
EPISODES_PER_MODEL = BLOCK_SIZE * BLOCKS_PER_MODEL  # 24
TOTAL_RESTART_MARKERS = BLOCKS_PER_MODEL * len(OFFICIAL_MODELS)  # 6
...
RUN_MODE_MARKER_FILENAME = ".prefill_screen_run_mode"


# ============================================================================
# Portable paths (derived from this file's own location, not hardcoded)
# Expected location: <PROJECT_ROOT>/new/scripts/prefill_screen/run_prefill_screen.py
# ============================================================================

SCRIPT_PATH = Path(__file__).resolve()
PROJECT_ROOT = SCRIPT_PATH.parents[3]
DEFAULT_SCHEDULE_DIR = PROJECT_ROOT / "new" / "runs" / "prefill_screen"


def default_output_dir(mode: str) -> Path:
    return PROJECT_ROOT / "new" / "runs" / "prefill_screen" / "results" / mode
...
def check_audit(audit_text: str, expected_fingerprint: object) -> list[str]:
    ...
    if not re.search(
        r"^design_version:\s*prefill-screen-v1\s*$", audit_text, re.MULTILINE
    ):
        errors.append(
            "audit report does not contain 'design_version: prefill-screen-v1'"
        )
    if not re.search(
        r"^total episodes \(all models\):\s*24\s*$", audit_text, re.MULTILINE
    ):
        errors.append(
            "audit report does not contain 'total episodes (all models): 24'"
        )
    ...
    if not is_valid_fingerprint_format(fp):
        ...
        if fp != OFFICIAL_FINGERPRINT:
            errors.append(
                ...
                f"frozen Prefill-Screen fingerprint {OFFICIAL_FINGERPRINT!r}"
            )
...
def load_and_validate_bundle(schedule_dir: Path) -> tuple[LoadedBundle | None, list[str]]:
    try:
        ...
        json_obj = load_json_bundle(paths["prefill_screen_schedule.json"])
        csv_fieldnames, csv_rows = load_csv_bundle(paths["prefill_screen_schedule.csv"])
        audit_text = load_audit_text(paths["prefill_screen_schedule_audit.txt"])
...
RUNNER_VERSION = "run_prefill_screen-v1"
...
class RealEnvironmentProbe:
    ...
    def gather(self, schedule_dir: Path) -> dict:
        ...
        file_hash_targets = {
            "run_prefill_screen.py": SCRIPT_PATH,
            "run_prefill_screen.sh": SCRIPT_PATH.parent / "run_prefill_screen.sh",
            "run_server.sh": SCRIPT_PATH.parent / "run_server.sh",
            "prefill_screen_schedule.json": schedule_dir / "prefill_screen_schedule.json",
            "prefill_screen_schedule.csv": schedule_dir / "prefill_screen_schedule.csv",
            "prefill_screen_schedule_audit.txt": schedule_dir / "prefill_screen_schedule_audit.txt",
        }
...
class FakeEnvironmentProbe:
    ...
    def __init__(self, env: dict | None = None) -> None:
        self.env: dict = env if env is not None else {
            ...
            "file_hashes": {
                "run_prefill_screen.py": "a" * 64,
                "run_prefill_screen.sh": "b" * 64,
                "run_server.sh": "c" * 64,
                "prefill_screen_schedule.json": "d" * 64,
                "prefill_screen_schedule.csv": "e" * 64,
                "prefill_screen_schedule_audit.txt": "0" * 64,
            },
        }
...
async def run_official_campaign(
    *,
    ...
    if resume:
        ...
        # integrity_manifest.json, .prefill_screen_run_mode) or unknown ones --
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="run_prefill_screen.py",
        description=(
            "Prefill-Screen runner (frozen exploratory campaign: schedule-bundle validation, "
            "resume-depth checks, self-test, dry-run, real execution of a "
            "single selectable smoke-test block, and the complete "
            "official 6-block/24-episode screening campaign). --official-run is "
            "enabled."
        ),
    )
    ...
    mode_group.add_argument(
        ...
        help="Really run the complete frozen screening campaign (6 server blocks, "
...
# ============================================================================
#
# Focused no-GPU/no-network checks live in the sibling prefill_screen_tests/
# package (prefill_screen_tests/selftest_runner.py and friends) -- see
# main() below for the lazy import. This module's own production logic
# above is otherwise byte-for-byte unchanged by that move.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.self_test:
        # The self-test suite lives in the sibling prefill_screen_tests/
        # package (moved out mechanically -- see the "Self-test" section
        # marker above). Register this already-executing module under
        # the stable name "run_prefill_screen" first, so that
        # the test package's import of `run_prefill_screen`
        # reuses this exact module object instead of re-executing this
        # file a second time under a separate module identity.
        sys.modules.setdefault("run_prefill_screen", sys.modules[__name__])
        from prefill_screen_tests.selftest_runner import run_self_test as _run_self_test
