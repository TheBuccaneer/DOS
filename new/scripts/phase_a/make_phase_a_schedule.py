...
Generates the final, frozen Phase-A episode schedule -- reproducibly,
self-validating, and only publishing output once every check has
passed. Does NOT execute any requests -- output is a schedule (CSV +
JSON) plus a human-readable audit report.

Design per model:
  - 2 states (offload0="low", offload12="high")
  - 2 victim concurrencies (4, 8)
  - 2 conditions (no_burst, fixed_burst)
  - N repeats (frozen at 5)
...
Block structure:
  ...
  run_phase_a.py (NOT by this generator), is:
      server start
  ...
  schedule therefore contains only the 80 regular episodes; the
...
The Phase A design is frozen. The CLI intentionally accepts only the
one official invocation (defaults, or the same values passed
explicitly) plus a free --output-dir override; see `validate_frozen_cli`.

Usage:
    python3 make_phase_a_schedule.py
    python3 make_phase_a_schedule.py --models llama qwen --repeats 5 \
        --seed 20260711 --output-dir /path/to/runs/phase_a
"""
...
# protocol that run_phase_a.py must follow:
...
# Frozen official invocation.
DEFAULT_MODELS = ("llama", "qwen")
DEFAULT_REPEATS = 5
DEFAULT_SEED = 20260711

# Portable default output directory, derived from this file's location.
# Expected location: <PROJECT_ROOT>/new/scripts/phase_a/make_phase_a_schedule.py
SCRIPT_PATH = Path(__file__).resolve()
PROJECT_ROOT = SCRIPT_PATH.parents[3]
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "new" / "runs" / "phase_a"
...
def build_audit_text(
    ...
    lines.append("--- stabilization protocol (documented for run_phase_a.py; "
    ...
    lines.append(
        ...
        "with a mix of old and new files. Before use, run_phase_a.py "
...
def write_and_replace_output_files(
    ...
    new phase_a_schedule.csv / .json / audit.txt files (e.g. a
    ...
    Consequently, callers (in particular run_phase_a.py) MUST NOT
    ...
    them, run_phase_a.py must at minimum:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the final, frozen Phase-A episode schedule "
...
def validate_frozen_cli(args: argparse.Namespace) -> str | None:
    """
    The Phase A design is frozen: --models/--repeats/--seed may be
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if frozen_error:
        ...
        print(
            "The Phase A design is frozen. Allowed invocation: "
